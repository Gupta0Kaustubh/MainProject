...
function UserForgotPassword() {
  ...
  const handleSubmit = (e) => {
    ...
    if (passChange.newpassword.length < 8) {
    ...
    } else if (!isStrongPassword(passChange.newpassword)) {
      toast.error(
      ...
      toast.error('Enter the new password in both fields the same !!');
    } else {
      const isUser = UserDetails.allUserData.find(
        (user) =>
          user.email === passChange.email && user.passwords == passChange.defaultpassword
      );
      if (isUser) {
        axios
